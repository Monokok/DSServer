...
const Lesson = ({ lessons, setLessons, removeLesson }) => { //состояние и два метода: деструктурирующее присваивание JavaScript и присвоение значений свойств локальным переменным
  ...
  useEffect(() => {
    ...
    const getLessons = async () => { //функция обратного вызова, которая вызывается React в подходящее время;
      const requestOptions = {
        method: "GET",
      };
      return await fetch("api/lessons", requestOptions)
  ...
  const deleteItem = async ({ id }) => { //объявление функции (асинхронной) в которую передаём id
    const requestOptions = { method: "DELETE", };
    return await fetch(
      `api/Lessons/${id}`,
      requestOptions
    ).then( //фнуукция вернёт промисс (из-за async) 
      (response) => {//response - ответ от сервера?
        // выполнится, когда от API придет ответ
        if (response.ok) { //структура вернувшегося и его поле ok
          removeLesson(id);
        }
      },
